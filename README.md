...
2026-09-25 の回は、シェルに依存しない grader で `crash-course` が +0.33（各 3 回）、`pr-reader-first` が +0.50、`book` が +0.28、対照ケースは差なし（過剰発火なし）でした。実行できなかったコードを報告したのは、スキルありで 6 回中 6 回、なしで 6 回中 1 回です。
